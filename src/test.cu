...
//sorting now happens on GPU
  ...
  //    printf("sortd\n");
	...
	uint64_t* d_vals;
	CUDA_CHECK(cudaMalloc((void**) &d_vals, nvals*sizeof(vals[0])));
	cudaMemcpy(d_vals, vals, nvals*sizeof(vals[0]));
	qf_insert_gpu(&qf, d_vals, 0, key_count, nvals, nslots,  QF_NO_LOCK);
	/*
	for (uint64_t i = 0; i < nvals; i++) {
...
//commenting because no file in GPU version
/*
